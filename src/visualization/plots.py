...
def plot_walk_forward_equity_curve(walk_forward_results, filename='plots/walk_forward/plot_walk_forward.png', title_suffix=''):
    """
...
def plot_monte_carlo_distribution():
    ...
    """
    base_data = load_data('c:/Users/rwydaegh/OneDrive - UGent/Desktop/Documents/investeren/The Unintended Consequence of Rebalancing/data/Return.csv', start_date='1997-09-10')
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('plots/other/plot_monte_carlo.png')
    plt.close()
    print("Monte Carlo plot saved to plots/other/plot_monte_carlo.png")
...
def plot_crisis_analysis_curves(data_with_signals_vix):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('plots/other/plot_crisis_analysis.png')
    plt.close()
    print("Crisis analysis plot saved to plots/other/plot_crisis_analysis.png")

def plot_full_history():
    """
    Generates and saves a plot of the full backtest period.
    """
    full_data = load_data('c:/Users/rwydaegh/OneDrive - UGent/Desktop/Documents/investeren/The Unintended Consequence of Rebalancing/data/Return.csv')
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('plots/other/plot_full_history.png')
    plt.close()
    print("Full history plot saved to plots/other/plot_full_history.png")
...
def plot_baseline_performance(data):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('plots/other/performance.png')
    plt.close()
    print("Baseline performance plot saved to plots/other/performance.png")
...
def plot_all_sensitivity_analyses(data_with_signals, data_with_signals_vix):
    ...
    plot_sensitivity_results(weight_df, 'Signal Weight (Threshold/Calendar)', 'Performance vs. Signal Weights', 'plots/sensitivity/plot_sensitivity_weights.png')
    ...
    plot_sensitivity_results(norm_df, 'Normalization Constant', 'Performance vs. Normalization Constant', 'plots/sensitivity/plot_sensitivity_norm.png')
    ...
    if data_with_signals_vix is not None:
        ...
        plot_sensitivity_results(vix_df, 'VIX Threshold', 'Performance vs. VIX Threshold', 'plots/sensitivity/plot_sensitivity_vix.png')
...
def plot_vix_strategy_performance(df):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('plots/hedged_equity/plot_vix_strategy_equity_curve.png')
    plt.close()
    print("VIX-filtered strategy equity curve saved to plots/hedged_equity/plot_vix_strategy_equity_curve.png")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('plots/hedged_equity/plot_vix_strategy_drawdowns.png')
    plt.close()
    print("VIX-filtered strategy drawdown plot saved to plots/hedged_equity/plot_vix_strategy_drawdowns.png")
...
def main():
    ...
    try:
        vix_data = pd.read_csv('c:/Users/rwydaegh/OneDrive - UGent/Desktop/Documents/investeren/The Unintended Consequence of Rebalancing/data/vix.csv', index_col='Date', parse_dates=True)
    ...
    if baseline_wf_results is not None:
        plot_walk_forward_equity_curve(baseline_wf_results, filename='plots/walk_forward/plot_walk_forward.png', title_suffix=' (Baseline Retail)')
    ...
    if data_with_signals_vix is not None:
        ...
        if vix_wf_results is not None:
            plot_walk_forward_equity_curve(vix_wf_results, filename='plots/walk_forward/plot_walk_forward_vix_filtered.png', title_suffix=' (VIX-Filtered)')
